            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">美食收藏展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="meishiName" id="meishiName" placeholder="美食名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <select name="kouweiTypes" id="kouweiTypes" lay-filter="kouweiTypes">
                                      ...
                                      <option v-for="item in kouweiTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <select name="dianzhiTypes" id="dianzhiTypes" lay-filter="dianzhiTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in dianzhiTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                        ...
                        <ul>
                            <li v-for="(item,index) in dataList" v-bind:key="index">
                                <div class="ng-prslist-doc">
                                    <div class="ng-prslist-img" @click="jump('../meishi/detail.jsp?id='+item.meishiId)">
                                        <a href="#" class="ng-prsliik">
                                          <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.meishiPhoto?item.meishiPhoto.split(',')[0]:''" />
                                          </span>
                                    ...
                                    <div class="ng-prslist-tit" @click="jump('../meishi/detail.jsp?id='+item.meishiId)"><span>{{item.meishiName}}</span></div>
                                    <div class="ng-prslist-tit" v-if="item.newMoney"><span style="color: red;">{{item.newMoney}} RMB</span></div>
                                    <div class="ng-prslist-tit" ><span style="color: red;" @click="deleteMeishiCollection(item.id)">删除 {{item.meishiName}} 收藏</span></div>
                                </div>
                            </li>
                        </ul>
                    </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 meishiCollectionContent 字段去除无用字符 如果字段长度大于60  就截取到60
            meishiCollectionContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            ,deleteMeishiCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('meishiCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#meishiName').val() != null && jquery('#meishiName').val() != "" && jquery('#meishiName').val() != "null" ) {
                param['meishiName'] = jquery('#meishiName').val();
            }
                                 
            var kouweiTypes = $("#kouweiTypes").find("option:selected").val();
            if (kouweiTypes != null && kouweiTypes != "" && kouweiTypes != "null") {
                            param['kouweiTypes'] = kouweiTypes;
                        }
                 
            var dianzhiTypes = $("#dianzhiTypes").find("option:selected").val();
            if (dianzhiTypes != null && dianzhiTypes != "" && dianzhiTypes != "null") {
                            param['dianzhiTypes'] = dianzhiTypes;
                        }
                                                                
            // 当前表
                                // 获取列表数据
            http.request('meishiCollection/list', 'get', param, function(res) {
...
// 级联表的字典表
       ...
       function kouweiTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=kouwei_types", 'get', {}, function (res) {
               if(res.code == 0){
       ...
       function dianzhiTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=dianzhi_types", 'get', {}, function (res) {
               if(res.code == 0){
